...
tests = [
    ...
    ('orders list reacts to live order invalidation', "elfar:orders:changed" in orders and 'useVisiblePolling(load, 15000)' in orders),
    ('open order card reacts to its live event', "elfar:orders:changed" in order_page and 'useVisiblePolling(refreshLocalOrder, 15000)' in order_page),
    ...
    ('Mini App consumes SSE and keeps fallback sync', 'consumeOrderEvents' in mini_api and 'consumeOrderEvents' in mini_app and 'delay = 20000' in mini_app),
