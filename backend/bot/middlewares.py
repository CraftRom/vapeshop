...
class PrivateOnlyMiddleware(BaseMiddleware):
    ...
    хендлери. Випадкове «Члвлв» отримувало «Щоб написати менеджеру, потрібне
    ...
    async def __call__(
        ...
        # Перемикачі в налаштуваннях: чи реагувати на ключові слова.
        #
        # Їх два, бо чати різні за призначенням. У клієнтській групі
        # миттєва відповідь на «яка доставка» — користь: людина отримує
        # її вночі й не чекає менеджера. У робочому чаті з замовленнями
        # та сама відповідь — шум: там працює команда, і обговорення
        # доставки щоразу викликало довідку про доставку.
        #
        ...
        # Пряме звернення працює за будь-якого перемикача: людина
        # покликала бота особисто, і мовчання у відповідь виглядало б як
        # поломка, а не як тиша за налаштуванням.
        if not allowed and not mentioned:
            return None
