...
class PrivateOnlyMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        # Перемикач у налаштуваннях: чи взагалі реагувати на ключові
        # слова в групах.
        #
        # Вимкнений — бот мовчить на будь-яке питання в чаті, поки до
        # нього не звернуться прямо. Пряме звернення лишається робочим
        # свідомо: людина покликала бота особисто, і мовчання у відповідь
        # виглядало б як поломка, а не як тиша за налаштуванням.
        #
        # Потреба звідси: у робочому чаті команди обговорення доставки
        # щоразу викликало довідку про доставку. Змістовно доречно,
        # по суті — шум посеред розмови людей.
        if not current().faq_public_enabled and not mentioned:
            return None
