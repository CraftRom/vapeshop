...
Скрипт перевіряє, що аргументи взагалі схожі на домени: прапорці (`-v`)
і посилання з розміткою (`[www.site](https://www.site)`) відхиляються з
підказкою. Без цього вони їдуть у certbot як значення для `-d`, і той
падає з невиразним `expected one argument`.

### Поки сертифіката немає

Порт 80 віддає сайт **сам**, а не лише перенаправляє на HTTPS. Це навмисно:
із самопідписаним сертифікатом браузер показує «З'єднання не конфіденційне»
і в магазин не пускає взагалі — краще віддати його незашифрованим, ніж
не віддати ніяк. HSTS у цей час теж вимкнений, інакше браузер запамʼятав би
домен як «тільки HTTPS» і заблокував би цей запасний шлях.

Після успішного випуску `certbot-init.sh` кладе два файли —
`nginx/redirect.d/force-https.conf` і `nginx/hsts.d/hsts.conf` — і порт 80
знову стає лише перенаправленням.

> **Якщо домен раніше жив на іншому хостингу з HSTS**, браузер міг
> запамʼятати заборону на HTTP і сам підмінятиме `http://` на `https://`.
> Перевірити й скинути: `chrome://net-internals/#hsts` → Delete domain
> security policies. На чужих пристроях це не скинути, тому справжній
> сертифікат усе одно потрібен якнайшвидше.

Продовження працює само: контейнер `certbot` перевіряє строк двічі на добу.

### Якщо впіймали ліміт Let's Encrypt

`too many certificates (5) already issued for this exact set of identifiers`
означає рівно те, що написано: вичерпано ліміт **на точний набір доменів**
за 168 годин. Три виходи:

```bash
# 1. Інший набір доменів — ліміт на нього окремий і, найімовірніше, вільний
...
Кожен сервіс пише два потоки: у stdout (його збирає docker) і у файл
`deploy/logs/<сервіс>.log`. Формат — один JSON на рядок, як у
serverless-платформ. Причина не в моді: рядок «замовлення 42 створено»
читає людина, але його неможливо відфільтрувати за клієнтом чи порахувати
відсоток помилок. JSON дає і те, і те.

```bash
cd /opt/elfar/deploy

# усі помилки за сьогодні
jq 'select(.level=="error")' logs/api.log

# найповільніші запити
jq -r 'select(.durationMs) | "\(.durationMs) \(.path)"' logs/api.log | sort -rn | head

# усе про один запит — номер є в заголовку X-Request-Id
jq 'select(.requestId=="a1b2c3")' logs/*.log

# невдалі спроби входу
jq 'select(.event=="auth.login.failed")' logs/api.log
```

Поля запиту: `requestId`, `method`, `path`, `host`, `ip`, `userAgent`,
`status`, `durationMs`. IP береться з `X-Forwarded-For` — інакше в журналі
була б адреса nginx, однакова для всіх.

Пароль у журнал не потрапляє ніколи: при невдалому вході пишеться лише
його довжина. Цього досить, щоб відрізнити помилку набору від перебору,
і замало, щоб журнал сам став витоком.

**Куди саме лягають файли.** У теку `deploy/data` — журнал, копії бази
й завантажені зображення в підкаталогах `logs`, `backups`, `media`.
Шлях не налаштовується жодною змінною: усередині контейнерів це завжди
`/data`, підкаталоги створює сам застосунок при першому звертанні.

Так зроблено після того, як три окремі змінні дали три способи помилитися:
одна одруківка в шляху — і застосунок мовчки писав нікуди, а сторінка
журналу показувала порожньо. Тепер помилитися нема де.

Стан тек видно в `/api/health` у полі `storage` і на сторінці «Журнал»,
коли записів немає.
...
`LOG_JSON=0` перемикає консоль на читабельний текст для розробки — у файл
JSON пишеться завжди, бо файли читають програми.

### Рівні

| Значення | Число | Що потрапляє в журнал |
|---|---|---|
| `NOTSET` | 0 | геть усе, включно з чужими бібліотеками |
| `DEBUG` | 10 | подробиці для розслідування |
| `INFO` | 20 | звичайний робочий журнал (за замовчуванням) |
| `WARNING` | 30 | лише те, що варте уваги |
| `ERROR` | 40 | лише помилки |
| `CRITICAL` | 50 | лише падіння |

Приймаються також синоніми `WARN`, `FATAL`, `TRACE` і **будь-яке число
0–100** — між рівнями інколи ставлять проміжне значення, щоб приглушити
конкретну бібліотеку, не зачепивши решту.

Незрозуміле значення не ковтається мовчки: застосунок бере `INFO` і
одразу пише про це в журнал, а `/api/settings/environment` показує
`LOG_LEVEL` як проблему. Мовчазне падіння на `INFO` — найгірший варіант:
людина виставила `DEBUG` заради розслідування, отримала звичайний журнал
