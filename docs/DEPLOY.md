...
Порт 80 віддає лише `/.well-known/acme-challenge/`, потрібний ACME-клієнту.
Усі інші HTTP-запити одразу перенаправляються на HTTPS: API, Mini App і
панель ніколи не передають токени або дані магазину незашифрованим каналом.
Для першого старту `certbot-init.sh` тимчасово створює самопідписаний
сертифікат, щоб nginx міг підняти 443 порт і пройти випуск справжнього TLS.

HSTS до успішного випуску вимкнений. Після отримання довіреного сертифіката
скрипт створює `nginx/hsts.d/hsts.conf`; лише тоді браузеру дозволено
закріпити режим HTTPS на довгий строк.
