...
`crontab`, ні зовнішнього планувальника. Він веде незалежні фонові контури:

1. **Запускає відкладені розсилки** з перевіркою приблизно кожні 15 секунд.
2. **Повторює невдалі записи в SalesDrive**, не блокуючи інші задачі.
3. **Страхує SalesDrive webhook фоновим read-sync** активних заявок. За
   замовчуванням інтервал 180 секунд; фінальні CRM-статуси перечитуються
   значно рідше, щоб історія не витрачала квоту `/api/order/list/`.
4. **Знімає бекап бази** раз на добу о заданій годині.
5. **Виконує retention/housekeeping** журналів, дампів і старих посилань на
   вкладення незалежно від того, чи був backup успішним.
...
`SCHEDULER_INTERVAL_SECONDS` задає лише частоту пробудження легкого циклу;
рекомендоване значення — **15 секунд**. Важкі контури мають власні gate-и,
тому CRM-read, backup і housekeeping не запускаються кожні 15 секунд. Навіть
якщо у старому `.env` лишилося значення `3600`, runtime обмежує tick зверху
15 секундами, щоб відкладені розсилки й retry не ставали годинними.

Для резервного SalesDrive read-sync використовуйте
`SALESDRIVE_BACKGROUND_REFRESH_SECONDS=180` або більше. Це страховка webhook,
а не основний live-канал: зменшувати значення до 60 секунд не варто через
квоти SalesDrive order-list API.
