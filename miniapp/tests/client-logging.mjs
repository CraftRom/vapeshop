...
check(api.includes("AUTH_FAILURE_EVENT = 'elfar:auth-failure'") && api.includes('signalAuthFailure(detail)'), '401 Mini App є термінальною подією, а не reconnect-loop')
check(app.includes("err?.authFailure || err?.status === 401"), 'realtime не перепідключається після втрати авторизації')
check(app.includes("!document.hidden && navigator.onLine !== false"), 'очікуваний disconnect прихованого WebView не засмічує warning-журнал')
