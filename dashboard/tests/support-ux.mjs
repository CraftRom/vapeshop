...
console.log('\n--- підтримка: багатосесійний inbox UX 1.28 ---')
...
check(page.includes('api.support.setStatus') && page.includes('api.support.remove'), 'чат можна закрити/відкрити та видалити вручну')
check(page.includes('window.confirm'), 'остаточне видалення захищене підтвердженням')
...
check(version.includes("APP_VERSION = '1.28.0'"), 'версія панелі 1.28.0')
